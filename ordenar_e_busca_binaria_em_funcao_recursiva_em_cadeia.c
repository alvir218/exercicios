...
int buscarNumero(int numeroProcurado, int inicio, int fim) {
	...
	if (vetor[meio] == numeroProcurado) {
		...
		if (vetor[meio] != numeroProcurado && meio == inicio && meio == fim) {
			...
			if (vetor[meio] > numeroProcurado) {
				fim = meio - 1;
				buscarNumero(numeroProcurado, inicio, fim);
			}
			else
				if (vetor[meio] < numeroProcurado) {
					inicio = meio + 1;
...
int ordenarEBuscar(int numeroProcurado, int inicio, int fim, int iteracaoi, int iteracaoj) {
	ordenarVetor(fim, iteracaoi, iteracaoj);
	buscarNumero(numeroProcurado, inicio, fim);
	return buscarNumero(numeroProcurado, inicio, fim);
}
